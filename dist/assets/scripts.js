(() => {
  ...
  var require_src3 = __commonJS({
    "node_modules/websocket-as-promised/src/index.js"(exports, module) {
      ...
      var WebSocketAsPromised3 = class {
        /**
      ...
      module.exports = WebSocketAsPromised3;
    }
  });
  ...
  // src/scripts.js
  var import_websocket_as_promised = __toESM(require_src3(), 1);
  ...
  var BulkScanner = class {
    ...
    // Websockets
    SetupWebsockets = () => {
      this.wsp = new import_websocket_as_promised.default("ws://localhost:49437", {
        packMessage: (data) => JSON.stringify(data),
        unpackMessage: (data) => JSON.parse(data),
        attachRequestId: (data, requestId) => Object.assign({ uid: requestId }, data),
        extractRequestId: (data) => data && data.uid
      ...
      this.wsp.onUnpackedMessage.addListener(async (ev2) => {
        switch (ev2.event) {
          case "reader_added":
            this.ReaderConnected = true;
            break;
          case "handle_added":
            this.HandleScannedHalo(ev2);
            break;
          case "exec_success":
            this.HandleScanSuccess(ev2);
            break;
          case "exec_exception":
            this.HandleScanFailure(ev2);
            break;
          default:
            console.log("default", ev2);
        }
      });
    };
    ...
    HandleScannedHalo = async (ev2) => {
      const res1 = await this.wsp.sendRequest({
        type: "exec_halo",
        handle: ev2.data.handle,
        command: {
          name: "get_pkeys"
        }
      });
      if (!res1.data?.res?.publicKeys)
        return;
    ...
    HandleScanFailure = (ev2) => {
      console.log("This scan failed", ev2);
    };
    VerifyChip = async () => {
      let keys = publicKeys ? parseKeysCli(publicKeys) : parseKeys(buf2hex2(staticHaloData));
      let primaryPublicKeyHash2 = keys["primaryPublicKeyHash"];
      keys["metadata"] = this.Els.metadata.value;
      keys["address"] = ethers.utils.computeAddress("0x" + keys["primaryPublicKeyRaw"]);
      const recordExists = this.Halos[primaryPublicKeyHash2] !== void 0;
      if (!recordExists) {
        this.Halos[primaryPublicKeyHash2] = keys;
        ...
        if (metadata.length > 0) {
          ...
          this.Halos[primaryPublicKeyHash2].sig = res;
